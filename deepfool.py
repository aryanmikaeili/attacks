...
def deepfool(model, ori_pointcloud, max_iter=50, overshoot=0.02, num_classes=10):
    i = 0

    x_pert = copy.deepcopy(ori_pointcloud)

    logits_ori, _, _ = model(ori_pointcloud)
    ...
    while k_pert == k_ori and i < max_iter:
        ...
        # clipped_gradients = torch.from_numpy(pert_final).to(device) / torch.norm(torch.from_numpy(pert_final).to(device), dim = 1)
        # clipped_gradients[clipped_gradients != clipped_gradients] = 0
        # clipped_gradients *= NN_mean

        # x_pert = ori_pointcloud + clipped_gradients * (1 + overshoot)
